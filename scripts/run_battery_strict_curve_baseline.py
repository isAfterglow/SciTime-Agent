...
def run_one_task(task_name: str, dataset_rel: str, target_col: str, models: list[str], out_root: Path):
    ...
    metrics_df.to_csv(out_dir / "strict_curve_metrics.csv", index=False)
    avg_metrics_df.to_csv(out_dir / "strict_curve_average_metrics.csv", index=False)
    predictions_df.to_csv(out_dir / "strict_curve_predictions.csv", index=False)
    ...
    with (out_dir / "strict_curve_manifest.json").open("w", encoding="utf-8") as f:
